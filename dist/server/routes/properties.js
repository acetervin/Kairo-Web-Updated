...
router.get('/', async (_req, res) => {
    try {
        ...
        if (properties.length > 0) {
            ...
            const imgRows = await pool.query('SELECT property_id, category, image_url FROM property_images WHERE property_id = ANY($1) AND (is_active IS NULL OR is_active = true) ORDER BY property_id, sort_order, id', [ids]);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (!categorizedImages ||
            ...
            const imageRows = await pool.query('SELECT category, image_url FROM property_images WHERE property_id = $1 AND (is_active IS NULL OR is_active = true) ORDER BY sort_order, id', [id]);
            ...
            categorizedImages = categorizedImages.map((item) => {
