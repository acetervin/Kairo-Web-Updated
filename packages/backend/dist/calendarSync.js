...
async function registerCalendarFeed(req, res) {
    try {
        ...
        const insertRes = await pool.query(`INSERT INTO calendar_sync (property_id, platform, external_calendar_url, is_active, created_at, updated_at)
...
async function syncCalendarFeed(req, res) {
    try {
        ...
        for (const ev of events) {
            ...
            await pool.query(`INSERT INTO blocked_dates (property_id, start_date, end_date, reason, source, external_id, created_at, updated_at, is_active)
         VALUES ($1,$2,$3,$4,$5,$6,NOW(), NOW(), true)`, [feed.property_id || feed.propertyId, startDate, endDate, 'external_booking', feed.platform, ev.uid]);
        }
        // Update lastSyncAt
...
async function syncFeed(feed, pool) {
...
async function getBlockedDates(req, res) {
    try {
        ...
        const blockedDates = rowsRes.rows.map((row) => ({
            id: row.id,
...
async function exportPropertyICal(req, res) {
    try {
        const propertyId = Number(req.params.propertyId);
        if (!propertyId)
            return res.status(400).send('invalid propertyId');
        const rowsRes = await pool.query('SELECT start_date, end_date, reason, source, id FROM blocked_dates WHERE property_id = $1 AND is_active = true ORDER BY start_date', [propertyId]);
        const lines = [];
