...
async function createApp() {
    ...
    try {
        ...
        // @ts-ignore
        const { registerCalendarFeed, syncCalendarFeed, getBlockedDates, exportPropertyICal, syncAllFeeds } = await Promise.resolve().then(() => __importStar(require('./calendarSync.js')));
        app.post('/api/calendar-sync', registerCalendarFeed);
        app.post('/api/calendar-sync/:id/sync', syncCalendarFeed);
        app.post('/api/calendar-sync/sync-all', async (_req, res) => {
            try {
