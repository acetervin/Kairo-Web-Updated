...
router.post('/login', loginProtectionMiddleware, async (req, res) => {
    const ipAddress = getClientIp(req);
    const userAgent = getUserAgent(req);
...
router.post('/refresh', async (req, res) => {
    ...
    try {
        ...
        const userResult = await pool.query('SELECT id, username, name, email, role, is_active FROM admin_users WHERE id = $1 AND is_active = true', [decoded.id]);
...
router.get('/stats', verifyToken, async (req, res) => {
    try {
        ...
        // Active bookings
        const bookingsResult = await pool.query("SELECT COUNT(*) as count FROM bookings WHERE status = 'confirmed'");
        // Total revenue (using total_amount column from schema)
        const revenueResult = await pool.query('SELECT COALESCE(SUM(CAST(total_amount AS DECIMAL)), 0) as total FROM bookings WHERE status = $1', ['confirmed']);
...
router.get('/users', verifyToken, async (req, res) => {
    try {
        const result = await pool.query(`SELECT id, username, name, email, role, is_active, created_at, last_login
       FROM admin_users
...
router.post('/users', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`INSERT INTO admin_users (username, password_hash, name, email, role)
       VALUES ($1, $2, $3, $4, $5)
...
router.put('/users/:id', verifyToken, async (req, res) => {
    ...
    try {
        ...
        let params = [sanitizedName, sanitizedEmail, role, is_active];
...
router.delete('/users/:id', verifyToken, async (req, res) => {
    try {
        ...
        const result = await pool.query('DELETE FROM admin_users WHERE id = $1 RETURNING id', [parseInt(id)]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'User not found' });
        }
...
router.post('/properties', verifyToken, async (req, res) => {
    try {
        const { name, description, location, category, price_per_night, max_guests, bedrooms, bathrooms, main_image_url, map_url, amenities } = req.body;
        const result = await pool.query(`INSERT INTO properties (
...
router.put('/properties/:id', verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, location, category, price_per_night, max_guests, bedrooms, bathrooms, main_image_url, map_url, amenities } = req.body;
        const result = await pool.query(`UPDATE properties SET
        name = $1,
        description = $2,
        location = $3,
        category = $4,
...
router.delete('/properties/:id', verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('DELETE FROM properties WHERE id = $1 RETURNING id', [parseInt(id)]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Property not found' });
        }
...
router.get('/properties/:id/images', verifyToken, async (req, res) => {
    try {
        ...
        const rows = await pool.query('SELECT id, category, image_url, alt_text, is_primary, sort_order, is_active FROM property_images WHERE property_id = $1 ORDER BY sort_order, id', [id]);
...
router.post('/properties/:id/images', verifyToken, async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO property_images (property_id, category, image_url, alt_text, is_primary, sort_order)
       VALUES ($1,$2,$3,$4,COALESCE($5,false),COALESCE($6,0))
       RETURNING id, category, image_url, alt_text, is_primary, sort_order`, [id, category || null, url, alt_text || null, is_primary || false, sort_order ?? 0]);
        res.status(201).json(result.rows[0]);
    }
    catch (error) {
...
router.put('/properties/:id/images/:imageId', verifyToken, async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE property_images SET
...
router.get('/bookings/:id/check-conflicts', verifyToken, async (req, res) => {
    try {
        ...
        const conflictCheck = await pool.query(`SELECT b.id, b.guest_name, bd.start_date, bd.end_date
       FROM bookings b
...
router.put('/bookings/:id', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const currentBooking = await client.query('SELECT * FROM bookings WHERE id = $1', [parseInt(id)]);
        ...
        if (isConfirming && !wasAlreadyConfirmed) {
            const conflictCheck = await client.query(`SELECT b.id, b.guest_name, bd.start_date, bd.end_date
         FROM bookings b
         JOIN blocked_dates bd ON b.id = bd.booking_id
         WHERE b.id != $1
           AND b.property_id = $2
        ...
        if (isCancelling && wasConfirmed) {
            ...
            await client.query(`UPDATE blocked_dates 
       ...
       SET status = COALESCE($1, status),
        ...
        if (isConfirming) {
            await client.query(`INSERT INTO blocked_dates (property_id, start_date, end_date, reason, source, booking_id, created_at, updated_at, is_active)
...
router.delete('/bookings/:id', verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('DELETE FROM bookings WHERE id = $1 RETURNING id', [parseInt(id)]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Booking not found' });
        }
