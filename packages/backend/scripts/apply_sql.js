#!/usr/bin/env node
// Small migration runner using pg. Reads DB_URL from .env and applies provided SQL file inside a transaction.
...
async function main() {
  ...
  try {
    ...
    // Execute the SQL script. pg supports sending multiple statements in one query string.
    const res = await client.query(sql);
    await client.query('COMMIT');

    // If the SQL ends with a SELECT, res will contain that result for the last statement.
    if (res && res.rows) {
